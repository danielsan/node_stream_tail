...
/********************************************************************************
 ...
 The following is all directly from the node_chat example
...
stream.get = function (path, handler) {
  getMap[path] = handler;
};

stream.listen = function (port, host) {
  server.listen(port, host);
  sys.puts("Server at http://" + (host || "127.0.0.1") + ":" + port.toString() + "/");
};

stream.close = function () { server.close(); };
...
stream.staticHandler = function (filename) {
  var body, headers;
  var content_type = stream.mime.lookupExtension(extname(filename));
...
stream.mime = {
  ...
  lookupExtension : function(ext, fallback) {
    return stream.mime.TYPES[ext.toLowerCase()] || fallback || 'application/octet-stream';
  },
